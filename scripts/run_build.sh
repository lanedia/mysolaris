...
function generate_email_alert()
{
	...
	if [ `hostname` == "ctp-1-rbm-bld" ]; then
		if [ "$compiled" = false ]; then
			cat ${RESULT_FILE} | mailx -s "(${SCM_GROUP}) ${DevGroup} : ${MY_BRANCH} ${EMAIL_SUBJECT}" $EMAIL_LIST
		else
			cat ${RESULT_FILE} | mailx -s "(${SCM_GROUP}) ${DevGroup} : ${MY_BRANCH} ${EMAIL_SUBJECT} $WARNINGS" $EMAIL_LIST
		fi
	else
...
function build_cgw_packages()
{
	...
	if [ "$generate_tag" == "true" ]; then
        ...
        if [ "$older_code" == "true" ]; then
            GenTag=""
            UpdateBuildId=""
            LogAndEcho "Using older code, not setting GenTag or UpdateBuildId, please check generated tags & Build Ids"
        fi
	...
	then
		...
		if [ "${create_patch}" == "true" ]; then
			...
			LogAndEcho "$CHARGING_HOME/source/build/makePatch $UpdateBuildId $GenTag -a -b TINScgwOTV -p $PATCH_RELEASE "
			$CHARGING_HOME/source/build/makePatch $UpdateBuildId $GenTag -a -b TINScgwOTV -p $PATCH_RELEASE >> ${RESULT_FILE} 
		fi
	else
		rm $HOME/$BASEPKGNAME
		ln -s $TOP $HOME/$BASEPKGNAME
...
function build_sma_packages()
{
	...
	if [ -f $HOME/releases/htdocs/$VERSION/prepaid.exe ]; then
            LogAndEcho "cp -p $HOME/releases/htdocs/$VERSION/prepaid.exe $SOURCE_ROOT/htdocs"
            cp -p $HOME/releases/htdocs/$VERSION/prepaid.exe $SOURCE_ROOT/htdocs
        else
            LogAndEcho "No prepaid.exe in $HOME/releases/htdocs/$VERSION/prepaid.exe, TINCccc package will not contain prepaid.exe" $RED
        fi
...
function build_pom_packages()
{
    ...
    LogAndEcho "Create TINPbase Patch $PATCH_NUM"
        LogAndEcho "$BASE_PROJ_DIR/in/scripts/development/dopatch.faster.PPIN -v $VERSION -p TINPbase -n $PATCH_NUM -J"
        $BASE_PROJ_DIR/in/scripts/development/dopatch.faster.PPIN -v $VERSION -p TINPbase -n $PATCH_NUM -J
    LogAndEcho "Create TINPtmplt Patch $PATCH_NUM"
        LogAndEcho "$BASE_PROJ_DIR/in/scripts/development/dopatch.faster.PPIN -v $VERSION -p TINPtmplt -n $PATCH_NUM -J"
        $BASE_PROJ_DIR/in/scripts/development/dopatch.faster.PPIN -v $VERSION -p TINPtmplt -n $PATCH_NUM -J
    fi
}
...
while getopts aBb:cf:G:g:hi:Llmn:p:RS:TtuV:v opt
...
do
	case ${opt} in
		...
		V)
			...
			;;
		l)
			LogAndEcho "  ${opt}  - USING OLDER scripts, from tags or site fixes" $RED
			older_code=true
			;;
		L)
			LogAndEcho "  ${opt}  - USING LEGACY CODE" $RED
			legacy_code=true
			;;
		u)
			# Do a fresh checkout from the relevant repository
			LogAndEcho "  ${opt}  - Cvs update instead of fresh checkout"
			#if false CONFIG_DIR will not be updated
			update_code=true
			;;	
		a)
		...
		f)
			multiple_branches=true
			export BRANCH_LIST=${OPTARG}
			LogAndEcho "  ${opt}  - Provide list of branches in the file - ${BRANCH_LIST}"
			;;
		h)
			display_help
			;;
		i)
			iterativeBuild=${OPTARG}
			iterativeBuild=`echo $iterativeBuild|tr '[:upper:]' '[:lower:]'`
			if [ "$iterativeBuild" == "iterative" ]; then
				LogAndEcho "  ${opt}  - Iterative build - $iterativeBuild"
				ITERATIVE_BUILD=true
			else
				LogAndEcho "  ${opt}  - Non Iterative build -$iterativeBuild"
				ITERATIVE_BUILD=false
			fi
			;;
		m)
			...
			;;
		R)
			LogAndEcho "  ${opt}  - Regression flag set for backup purposes "
			# if run_regression is set, assume we want to checkout and compile the code for each branch
			run_regression=true
			automated=true
			checkout_code=true
			compile_code=true
			...
			;;
		p)
			# Set when packaging the group s/w.
			# In Solaris we have both base & patch pkgs, so we specify the type.
			pkgType=${OPTARG}
			pkgType=`echo $pkgType|tr '[:upper:]' '[:lower:]'`
			if [ "$pkgType" == "base" ]; then
				create_base_patch=true
				...
				LogAndEcho "Invalid Pkg type"
				exit 1
			fi
			LogAndEcho "  ${opt}  - Create $pkgType Package(s) "
			;;	
		g|G)
		...
		v)
			...
			;;
		t)
			LogAndEcho "  ${opt}  - Allow tagging "
			generate_tag=true
			;;	
		T)
			LogAndEcho "  ${opt}  - Use Tag instead of branch (http://saturn/svnrepos/prepaid/)"
			use_tag=true
			;;	
		*)
...
export create_patch
export create_base_patch
export ITERATIVE_BUILD
export BINARYRELEASE
if [ "$SCM_GROUP" == "ADC" ]; then
    export RESULT_DIR=$HOME/AutomatedBuildReports/
fi
if [ ! -d "$RESULT_DIR" ];
then
	mkdir -p $RESULT_DIR
...
i=0
if [ "$multiple_branches" == "true" ]; then
	while read line
	do
		if [[ "$line" =~ ^[^#]*= ]]; then
			branch[i]=${line%%=*}
	...
	branch[i]=$single_branch
	((i++))
fi

LogAndEcho "Num branches to run: ${#branch[@]}"
# Loop through each branch
# If set, checkout the code
# If set, compile the code
count=0
while [ $count -lt ${#branch[@]} ]
do
	export branch="$(trim ${branch[$count]})"
	if [ "$DevGroup" == "pom" ]; then
	...
	initial_bld_env="$HOME/AutomationEnvironments/$DevGroup.bashrc_$branch"
	...
	if [ -f $ENV_DIR/.bashrc_$DevGroup ]; then
            if [ -f $initial_bld_env ]; then
		LogAndEcho "$initial_bld_env already exists, not overwriting" $RED
            else
		cp $ENV_DIR/.bashrc_$DevGroup $initial_bld_env
		sed -e "s/CCS_BRANCH/$branch/" \
			$initial_bld_env > $initial_bld_env.copy
		cp $initial_bld_env.copy $initial_bld_env
		rm $initial_bld_env.copy
            fi
	else
	...
	export BUILD_ENV=$initial_bld_env
	source $BUILD_ENV
	...
	if [ "$checkout_code" == "true" ]; then
		if [ "$legacy_code" == "true" ]; then
			if [ "$DevGroup" == "db" ]; then
				checkout_git
			elif [[ "$DevGroup" == "voice" || "$DevGroup" == "brt" ]]; then
				checkout_svn
			elif [ "$DevGroup" == "sma" ]; then
				checkout_ccs_cvs $branch
			elif [ "$DevGroup" == "pom" ]; then
				checkout_pom_cvs $branch
			elif [ "$branch" == "Cgw_TIP" ]; then
				checkout_ccs_cvs
			else
				checkout_ccs_cvs $branch
			fi
		...
		EMAIL_SUBJECT="Checked out only"
	...
	if [ $HOSTNAME == "ctp-1-rbm-bld" ]; then
        export BINARY_RELEASE="NIGHTLY BUILD"
	else
        export BINARY_RELEASE="Development BUILD $HOSTNAME"
	fi
    compile_status=true
	LogAndEcho "Patch # = ${PATCH_RELEASE}" 
	if [ "$compile_code" == "true" ]; then
		compile_ccs
		if [ "$ITERATIVE_BUILD" == "false" ] ; then
			check_warnings
		fi
		display_seperator
	fi
